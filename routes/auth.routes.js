...
router.get('/refresh-db', async (req, res) => {
  try {
    const SOURCE_DB = 'ConnectWA/ConnectWA';
    const TARGET_DB = 'ConnectWA-node';
    const DB_USER = 'ConnectWA_node_user';
    ...
    const commands = [
      ...
      `mongorestore --uri "mongodb://${DB_USER}:${encodeURIComponent(DB_PASS)}@${DB_HOST}:${DB_PORT}/${TARGET_DB}?authSource=ConnectWA" --drop "${backupPath}"`,

      `mongorestore --host ${DB_HOST} --port ${DB_PORT} --username ${DB_USER} --password "${DB_PASS}" --authenticationDatabase ConnectWA --db ${TARGET_DB} --drop "${backupPath}"`
    ];
